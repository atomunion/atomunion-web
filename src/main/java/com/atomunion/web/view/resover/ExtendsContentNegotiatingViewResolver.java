package com.atomunion.web.view.resover;
 ...
 */
public class ExtendsContentNegotiatingViewResolver extends
		ContentNegotiatingViewResolver {

	private static final Logger LOG = LoggerFactory
			.getLogger(ExtendsContentNegotiatingViewResolver.class);
	...
	public View resolveViewName(String viewName, Locale locale)
		...
		for (View candidateView : candidateViews) {
			MediaType viewMediaType = MediaType.parseMediaType(candidateView
					.getContentType());
		...
		if (!views.isEmpty()) {
			...
			if (LOG.isDebugEnabled()) {
				LOG.debug("Returning [" + view
						+ "] based on requested media type '" + mediaType + "'");
			}
			return view;
		} else {
			return null;
