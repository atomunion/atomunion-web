...
public class NavigationTagV2 extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			for (int i = 1; i <= number
					&& startIndex <= onePage.getContext().getPageCount(); startIndex++, i++) {
				...
				if (optimize && firstModel != null && lastModel != null) {
					if (startIndex < currIndex) {
						...
						offset = (startIndex - currIndex + 1)
								* onePage.getContext().getPageSize();
				...
				writer.print("<a href=\"" + pageUrl + "\">" + startIndex
						+ "</a>&nbsp;");
			}
			if (onePage.isHasNext()) {
				String nextUrl = append(url, "pn", onePage.getIndex() + 1);
				if (optimize && lastModel != null) {
					nextUrl = append(nextUrl, PageUtil.getIdName(lastModel),
							PageUtil.getIdValue(lastModel));
				}
				if (onePage.getContext().getPageCount() - onePage.getIndex() > 2) {
					writer.print("<a href=\""
							+ append(url, "pn", onePage.getContext()
									.getPageCount()) + "\">"
							+ onePage.getContext().getPageCount()
							+ "</a>&nbsp;");
				}
				writer.print("<a href=\"" + nextUrl + "\">下一页</a>");
			}
			writer.print("&nbsp;(共" + onePage.getContext().getTotal()
					+ "条记录)<br/>");
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	private String resolveUrl(String url,
		...
		url = url.replaceAll("&pn=\\d*", "").replaceAll("pn=\\d*", "")
				.replaceAll("&pre=true", "").replaceAll("&id=\\d*", "")
				.replaceAll("id=\\d*", "");
